...
const Response: React.FC = () => {
  ...
  return (
    <div id="testimonial" className="flex flex-col justify-center items-center py-16 px-4">
      <h2 className="font-clash-display text-center font-medium mt-12 text-4xl">
        Program phone agents to automate any task
      </h2>
      <div className="flex flex-col justify-center items-center mt-10 border border-[#FF853C] w-full max-w-2xl rounded-lg shadow-lg p-6">
        <div className="w-full font-clash-display">
          <Slider {...settings}>
            {slides.map((slide, index) => (
              <div key={index} className="flex justify-center items-center p-4">
                <div className="flex flex-col justify-center items-center text-center space-y-4">
                  <img src={slide.imgSrc} alt={slide.imgAlt} className="h-16 w-16" />
                  <h3 className="text-2xl font-semibold">{slide.title}</h3>
                  <p className="text-md">
                    {slide.description}
                  </p>
                  <button className='bg-[#ff9345] px-2 py-2 border rounded-xl hover:scale-95'>Try Now</button>
                </div>
              </div>
            ))}
          </Slider>
